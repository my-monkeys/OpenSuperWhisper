...
final class SurroundingContextTests: XCTestCase {
    ...
    /// two are usually in different fields. The text has to ride on the clip, the way the app
    /// and the model already do, or the queue hands the older clip whatever the newer one was
    /// started in. (parallel-recording)
    @MainActor
    func testEachClipIsTranscribedWithItsOwnField() async {
        ...
        final class Seen: @unchecked Sendable { var fields: [String?] = [] }
        let seen = Seen()

        let pipeline = DictationPipeline.shared
        pipeline.transcribeOverride = { _, settings in
            seen.fields.append(settings.focusedText)
            return "text"
        }
        defer { pipeline.transcribeOverride = nil }

        for (i, field) in ["the bonfire was Bunny's idea", "Dear Charity,"].enumerated() {
            pipeline.enqueue(
                tempURL: URL(fileURLWithPath: "/tmp/osw-field-test-\(i).wav"),
                startedAt: Date(),
                streamedFallback: "",
                context: DictationPipeline.ContextSnapshot(focusedText: field),
                modelOption: nil)
        }
        ...
        XCTAssertEqual(seen.fields, ["the bonfire was Bunny's idea", "Dear Charity,"])
    }
}
